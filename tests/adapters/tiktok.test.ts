import { TikTokAdapter } from '../../src/posting/adapters/tiktok';

describe('TikTokAdapter', () => {
  let adapter: TikTokAdapter;

  beforeEach(() => {
    adapter = new TikTokAdapter('client-key', 'client-secret');
  });
  ...
  describe('auth URL', () => {
    it('should generate valid OAuth URL', () => {
      const url = adapter.getAuthUrl('http://localhost:3000/oauth/callback');
      expect(url).toContain('tiktok.com/oauth/authorize');
  ...
  describe('privacy mapping', () => {
    it('should map public to PUBLIC_TO_EVERYONE', () => {
      expect(adapter.mapPrivacyLevel('public')).toBe('PUBLIC_TO_EVERYONE');
    });

    it('should map friends to FRIENDS_ONLY', () => {
      expect(adapter.mapPrivacyLevel('friends')).toBe('FRIENDS_ONLY');
    });

    it('should map private to PRIVATE', () => {
      expect(adapter.mapPrivacyLevel('private')).toBe('PRIVATE');
    });

    it('should default to PUBLIC_TO_EVERYONE', () => {
      expect(adapter.mapPrivacyLevel()).toBe('PUBLIC_TO_EVERYONE');
      expect(adapter.mapPrivacyLevel('unknown')).toBe('PUBLIC_TO_EVERYONE');
    });
  });
});
